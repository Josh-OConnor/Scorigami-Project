...
<nav class="navbar">
    ...
    $q1 = "SELECT * FROM `Cardinals` WHERE `Scorigami` = 1 AND `W-Team` = 'Arizona Cardinals' ";
    ...
    $q = "SELECT * FROM `Cardinals` WHERE `Scorigami` > 1 ORDER BY `Scorigami` ASC";
    $r = mysqli_query($dbc, $q) or die('Query error: ' . mysqli_error($dbc));
    $num = mysqli_num_rows($r);

    if ($num > 0) {
        echo "<h1>Cardinals Games</h1>";
        echo "<table>";
        echo "<tr><th>Scorigami</th><th>Date</th><th>W-Team</th><th>L-Team</th><th>W-Score</th><th>L-Score</th></tr>";
